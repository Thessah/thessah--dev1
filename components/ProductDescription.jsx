        ...
        <div className="my-8">
            ...
            <div className="bg-white border border-gray-200">
                ...
                <div className="p-8">
                                        ...
                                        {/* Rating Overview - Compact, Tight Layout */}
                                        <div className="mb-8">
                                            <div className="flex flex-col items-start gap-0 pb-4 border-b border-gray-200 max-w-sm">
                                                <div className="text-4xl font-bold text-gray-900 leading-none">{averageRating}</div>
                                                <div className="flex mb-0.5 mt-1">
                                                    {Array(5).fill('').map((_, i) => (
                                                        <StarIcon
                                                            key={i}
                                                            size={20}
                                                            fill={i < Math.round(averageRating) ? (i === 4 ? '#388e3c' : '#43a047') : '#E0E0E0'}
                                                            className="text-transparent"
                                                        />
                                                    ))}
                                                </div>
                                                <div className="text-xs text-gray-600 mb-1 mt-0.5">
                                            ...
                                            <div className="flex flex-col gap-0.5 mt-2">
                                                {[5, 4, 3, 2, 1].map((star) => {
                                                    ...
                                                    if (star === 4) barColor = '#8bc34a';
                                                    if (star === 3) barColor = '#ffc107';
                                                    if (star === 2) barColor = '#ff9800';
                                                    if (star === 1) barColor = '#e53935';
                                                    return (
                                                        <div key={star} className="flex items-center gap-1.5 min-h-[22px]">
                                                            <span className="text-sm font-medium text-gray-700 w-5 flex items-center justify-end">
                                                                <StarIcon
                                                                    size={16}
                                                                    fill={percentage > 0 ? barColor : '#E0E0E0'}
                                                                    className="inline-block mr-0.5 text-transparent"
                                                                />
                                                                {star}
                                                            </span>
                                                            <div className="flex-1 bg-gray-200 h-2 rounded-full overflow-hidden max-w-xs">
                                                                <div
                                                                    style={{ width: `${percentage}%`, background: percentage > 0 ? barColor : '#E0E0E0' }}
                                                                    className="h-full transition-all duration-300"
                                                                />
                                                            </div>
                                                            <span className="min-w-[28px] text-right text-xs text-gray-600">{percentage}%</span>
                                                        </div>
                                                    );
                                                })}
                                            </div>

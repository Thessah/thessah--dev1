  ...
  switch (status) {
    case 'ORDER_PLACED':
      return sendOrderConfirmationEmail({
        email, name, orderId: order._id, total: order.total, orderItems: order.orderItems, shippingAddress: order.shippingAddress, createdAt: order.createdAt, paymentMethod: order.paymentMethod
      });
    case 'CONFIRMED':
...
export async function sendOrderDeliveredEmail({ email, name, order }) {
  ...
  const html = `
    ...
    <div style="padding:32px;background:#f8f9fa;">
      <h2>Hi ${name || 'there'}!</h2>
      <p>Thank you for shopping with Thessah. If you have any feedback or need support, reply to this email.</p>
      <p style="margin-top:30px;">Order ID: <b>${order._id.toString().slice(-8).toUpperCase()}</b></p>
    </div>
  `;
  return sendMail({ to: email, subject, html });
...
export async function sendMail({ to, subject, html }) {
  ...
  if (resend) {
    ...
    try {
      const fromEmail = process.env.EMAIL_FROM || 'noreply@thessah.com';
      const fromName = process.env.EMAIL_FROM_NAME || 'Thessah';
...
export async function sendWelcomeEmail(email, name) {
  const subject = 'Welcome to Thessah! 🎉';
  const html = `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      <div class="container">
        <div class="header">
          <h1>Welcome to Thessah!</h1>
          <p>Your journey to amazing products starts here</p>
        </div>
        <div class="content">
          <h2>Hi ${name || 'there'}! 👋</h2>
          <p>Thank you for creating an account with Thessah. We're excited to have you as part of our community!</p>
...
export async function sendOrderConfirmationEmail(orderData) {
  const { email, name, orderId, total, orderItems, shippingAddress, createdAt, paymentMethod } = orderData;
  ...
  const html = `
    ...
    <body>
      <div class="container">
        <div class="header">
          <img src="https://res.cloudinary.com/dyccpu74t/image/upload/v1765529523/Asset_6_q7xkxg.png" alt="Thessah Logo" class="header-logo" />
          <h1>✅ Order Confirmed!</h1>
        ...
        <div class="content">
          ...
          ` : ''}
          <center>
            <a href="https://thessah.com/track-order" class="button" style="background:#3b82f6;color:#fff;padding:14px 36px;text-decoration:none;border-radius:8px;font-weight:bold;font-size:1rem;letter-spacing:0.5px;box-shadow:0 2px 8px rgba(40,116,240,0.08);transition:background 0.2s;">Track Order</a>
          </center>
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <center>
            <a href="https://thessah.com/track-order" class="button" style="background:#3b82f6;color:#fff;padding:12px 30px;text-decoration:none;border-radius:8px;font-weight:bold;display:inline-block;">Track Order</a>
          </center>
